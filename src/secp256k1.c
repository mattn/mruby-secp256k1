...
secp256k1_mrb_pubkey(mrb_state *mrb, mrb_value obj)
{
  ...
  int ret = secp256k1_ec_pubkey_create(context, &pubkey, secretkey_buf);
  if (ret == 0) {
...
secp256k1_mrb_sign(mrb_state *mrb, mrb_value obj)
{
  ...
  int ret = secp256k1_ecdsa_sign(context, &signature, message_buf, secretkey_buf,
                             NULL, NULL);
  if (ret == 0) {
...
secp256k1_mrb_verify(mrb_state *mrb, mrb_value obj)
{
  ...
  secp256k1_pubkey pubkey;
  int ret = secp256k1_ec_pubkey_parse(context, &pubkey, pubkey_buf, pubkey_len);
  if (ret == 0) {
